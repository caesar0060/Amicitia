...
public class E_Dragon : EnemyBase
{
	#region Properties
	#endregion
	// ------------------------------------------------------------------------------------										Debug用
	...
	void Start () {
		...
		string skillDate = GetSKillDate ("Dragon_Skill.json");
	...
	{
        this.GetComponentInChildren<Animator>().SetTrigger("isAttack");
        StartCoroutine(MagicDamage(_target, skillList[1], 1f, "Prefabs/Magic/meteo", 2.5f));
	}
	/// <summary>
    /// ハーガル
    /// スロウ状態にする
